...
- **Artifact:** a `doc-detective-report` artifact is uploaded with the JSON results (`doc-detective-results.json`), the Markdown summary (`doc-detective-summary.md`), and — when Doc Detective produces one (4.10.0+) — the HTML report. Download it from the run's summary page. When the action runs more than once in a workflow (for example in a build matrix), the artifact name is suffixed with the job name and runner OS (e.g. `doc-detective-report-pass-Linux`) so each run's report is distinct.