...
The action also attaches a Markdown summary (Doc Detective's own `markdown` reporter output) to the job summary page — no configuration required. To request it, the action passes `--reporters terminal json markdown`, which overrides (not merges with) a `reporters` list set in your own Doc Detective config file. This needs the Doc Detective 4.20+ series and doesn't happen for `version: 2.x`, which doesn't understand that flag. An unset/empty `version` (which runs whatever Doc Detective is already resolvable locally, e.g. via `npm link` — the action can't tell in advance whether that build supports the flag) is handled the same way: the action doesn't request the reporter, but still attaches whatever `doc-detective-summary.md` your build produces on its own (e.g. via its own default reporters or your Doc Detective config), if any.
