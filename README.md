...
- **Artifact:** an artifact named `doc-detective-report-<job>-<runner-os>` (e.g. `doc-detective-report-pass-Linux`) is uploaded with the JSON results (`doc-detective-results.json`), the Markdown summary (`doc-detective-summary.md`), and — when Doc Detective produces one (4.10.0+) — the HTML report. Download it from the run's summary page. The job and runner-OS suffix (from the `GITHUB_JOB` and `RUNNER_OS` variables, which GitHub always sets) keeps each report distinct when the action runs more than once in a workflow, such as in a build matrix.